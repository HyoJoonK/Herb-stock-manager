...
렌더러가 `InventoryManager`를 직접 `new`하여 사용하므로, 메인 프로세스와 렌더러 프로세스 사이에 데이터 관련 IPC는 없습니다. IPC는 오직 **자동 업데이트 상태 전달**(`update-status`, `check-for-updates-manual`), **앱 버전 조회**(`get-app-version`), **렌더러 초기화 완료 신호**(`renderer-init-complete` — DB 로드·첫 렌더링 완료 시 렌더러→메인, 메인 윈도우 노출 트리거)에만 쓰입니다.
...
- `WindowManager`: 스플래시(프레임 없음/투명)·메인 윈도우 생성과 수명, 렌더러 상태 전송(`sendStatusToWindow`/`sendStatusToSplash`). 메인 윈도우는 `ready-to-show`가 아니라 렌더러의 `renderer-init-complete` IPC 신호(DB 로드+첫 렌더링 완료)를 받은 뒤에 노출합니다(15초 타임아웃 안전장치, 노출 직전 스플래시에 100% 표시 후 200ms 지연).
